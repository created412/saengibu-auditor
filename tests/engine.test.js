...
test('추상 낱말 앞에 구체적 수식어가 있으면 빈 표현이 아니다', () => {
  const ok = '“지리학자의 인문여행(이영민)”을 읽고 장소와 문화적 맥락의 관계, 여행에서 만나는 전이적 장소와 경계의 의미를 탐구함. 또한 직접 답사한 순천만 갯벌의 형성과 생태적 가치를 조사하고 습지가 기후변화 완화와 생물다양성 유지에 미치는 영향을 분석함.';
  assert.ok(!E.audit(ok).issues.some((i) => i.type === 'vague'), '앞에 대상이 밝혀진 문장을 추상 표현으로 지적함');
  // 앞이 막연하면 그대로 지적한다
